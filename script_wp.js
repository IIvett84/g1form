(function ($) {
  if ($("html").attr("lang") === "en-US") defaultLang = 0;
  else defaultLang = 1; //for language choose
  ...
  $(document).ready(function () {
    ...
    if (defaultLang) clearText = "Törlés";
    else clearText = "Delete";
  ...
  $(document).ready(function () {
      // Kezdeti állapot: engedélyezzük a csomag választását
      $('select[name="pack"] input, textarea')
        .prop("disabled", false)
        .parent()
        .removeClass("padder2 disab");
    
      // Eseménykezelő, ami reagál a csomag választására
      $(document).on("change", 'select[name="pack"]', function () {
        var selectedPack = $(this).find("option:selected").val();
    
        // Először eltüntetjük az esetleges korábbi mezőket
        $(".checkbox-holder .padder2 disab").remove();
        $(".company-holder .padder2 disab").remove();
    
        // Ha a felhasználó nem választott csomagot
        if (selectedPack === "") {
          $(this).addClass("error");
          return;
        } else {
        ...
        // Csomag alapján további mezők (pl. cégnév vagy checkbox) megjelenítése
        if (selectedPack === "Champion Mini" || selectedPack === "Champion Basic") {
          // Ha a csomag "Champion Mini" vagy "Champion Basic", nem szükséges extra mező
          $(".checkbox-holder .padder2").remove();
          $(".company-holder .padder2").remove();
        }
    
        if (selectedPack === "Champion Advance" || selectedPack === "Champion Grand Prix") {
          // Ha a csomag "Champion Advance" vagy "Champion Grand Prix", megjelenhet egy checkbox
          var chkName = "Feldobox kuponnal rendelkezem";
          var chkInput =
            '<div class="padder2 disab">' +
            '<input disabled="" class="dontcheck" type="checkbox" id="feldobox" name="feldobox" title="' +
            chkName +
            '" />' +
            '<label class="neutral" for="feldobox">' +
            chkName +
            "</label>" +
            "</div>";
        ...
        if (selectedPack === "Champion Lemans") {
          // Ha a csomag "Champion Lemans", akkor cégnév mező jelenhet meg
          var cmpName = "Cégnév";
          var extraInput =
            '<div class="padder2 disab">' +
            '<label for="company"><i class="fa fa-gear"></i><span>' +
            cmpName +
            "</span></label>" +
            '<input disabled="" class="labeled label-fader" type="text" id="company" name="company" title="' +
            cmpName +
            '" />' +
            "</div>";
  ...
  $(document).on("change", 'select[name="pack"]', function () {
    var selPack = $(this).find("option:selected").val(); // Kiválasztott csomag

    // Ha nincs választva csomag
    if (selPack === "") {
      // Letiltjuk az összes input mezőt, gombot, textarea-t
      $("input, textarea, button")
        .prop("disabled", true)
        .parent()
        .removeClass  if (selPack === "") {
      // Letiltjuk az összes input mezőt, gombot, textarea-t
      $("input, textarea, button")
        .prop("disabled", true)
        .parent()
        .addClass("disab");("disab");

      $(this).addClass("error"); // Hibaállapot a legördülő menünek
      $(".pack-loader-opener").fadeOut(); // Ha van, eltüntetjük a loader-t
    } else {
      $(this).removeClass("error"); // Ha van választás, eltávolítjuk a hibát

      // A csomag választás alapján jeleníthetünk meg extra mezőket
      $(".checkbox-holder .padder2").remove(); // Eltávolítjuk az előző checkboxokat
      $(".company-holder .padder2").remove(); // Eltávolítjuk az előző cégnév mezőt

      // Csak a csomagok szerint jelennek meg extra mezők
      if (selPack === "Champion mini" || selPack === "Champion basic") {
        // Ha "Champion Mini" vagy "Champion Basic" van kiválasztva, nem kell további mező
        $(".checkbox-holder .padder2").remove();
        $(".company-holder .padder2").remove();
      } else if (
        selPack === "Champion advance" ||
        selPack === "Championship Grand Prix Race"
      ) {
        // Ha "Champion Advance" vagy "Champion Grand Prix" van kiválasztva, Feldobox checkbox
        var chkName = "Feldobox kuponnal rendelkezem";
        var chkInput =
          '<div class="padder2 disab">' +
          '<input disabled="" class="dontcheck" type="checkbox" id="feldobox" name="feldobox" title="' +
        ...
        $(".checkbox-holder").append(chkInput); // Hozzáadjuk a checkboxot
      } else if (selPack === "Championship Le Mans Race") {
        // Ha "Champion Lemans" van kiválasztva, cégnév mező
        var cmpName = "Cégnév";
        var extraInput =
          '<div class="padder2 disab">' +
          '<label for="company"><i class="fa fa-gear"></i><span>' +
        ...
        $(".company-holder").append(extraInput); // Hozzáadjuk a cégnév mezőt
      }

      // Itt töröljük a dátumot és időt, ha szükséges (pl. a naptár és idő picker alaphelyzetbe állítása)
      date_picker.clear();
      $(".datepicker").parent().find("label span").css({ opacity: 1 });
  ...
  $('input[name="person"]').attr({
    min: packs[selPack].min,

    max: packs[selPack].max,
  });
  ...
  if (selPack !== "Csomag nélkül") {
    ...
    if (defaultLang) kidsOrgName = "Létszám";
    else kidsOrgName = "Persons";
    ...
    if (selType == "Gyermek futam") {
      ...
      if ($(".adults-holder .padder2").length === 0) {
        ...
        extraInput +=
          '<label for="adults"><i class="fa fa-users"></i><span>' +
          adltName +
          "</span></label>";

        extraInput +=
          '<input disabled="" class="labeled label-fader dontcheck" type="number" min="0" max="100" id="adults" name="adults" title="' +
          adltName +
          '" />';
  ...
  for (
    var counter = 0;
    counter < packs[selType][selPack].disable.length;
    counter++
  ) {
...
$(document).on("change", ".datepicker", function () {
  ...
  if (selDay !== null) {
    ...
    if (selDate === "2017-12-24" || selDate === "2017-12-25") {
      ...
      if (
        $('select[name="type"]').find("option:selected").val() ===
        "Gyermek futam"
      )
        maxTime = [16, 0];
      else maxTime = defaultMaxTime;
...
$(document).on("submit", ".booking", function () {
  ...
  if (error) {
    ...
    if (defaultLang) tomail = packs[selPack].addressHu;
    else tomail = packs[selPack].addressEn;

    //tomail = 'vityak@freemail.hu';

    formData = form.serialize();

    if (defaultLang) title = "Új bejelentkezés";
    else title = "Új bejelentkezés (ENG)";

    /*str = selType.replace(" futam", "");

      title += " - " + str;*/

    if (selPack != "Csomag nélkül") title += " - " + selPack;

    //feldobox check

    if ($(".checkbox-holder .padder2").length !== 0) {
      if ($("#feldobox").is(":checked")) title += " - Feldobox";
    }

    sendData =
      "send_type=booking&title=" +
      title +
      "&address=" +
      tomail +
      "&" +
      ...
      .done(function (msg) {
        if (msg === "1") {
          ...
          if (defaultLang) {
            ...
            html =
              "Köszönjük foglalási igényét, amelynek feldolgozását hamarosan megkezdjük! ";

            html += "Tájékoztatjuk, hogy ez még nem minősül foglalásnak! ";

            html +=
              "Kollégáink 24 órán belül az Ön által megadott e-mail címre visszaigazolást küldenek, amelyben megerősítik bejelentkezését.<br /><br />";

            html +=
              "Kérjük, amennyiben nem kap visszaigazolást tőlünk, keressen fel minket elérhetőségeink valamelyikén!<br /><br />";

            html +=
              "A foglalás csak visszaigazolással érvényes, kérjük szíves türelmét!<br />Köszönettel: G1 csapata";
          } else {
            ...
            html =
              "Thank you for your booking request! This is not a fixed reservation yet! ";

            html +=
              "Our colleagues are going to confirm your booking request via e-mail in 24 hours.<br /><br />";

            html +=
              "If you do not receive the confirmation from us, please contact us!<br /><br />";

            html +=
              "Booking is only valid with written confirmation!<br />Thanks: G1 team";
          }
          ...
          if (defaultLang) {
            ...
            toHtml +=
              "A foglalási igény elküldése sikertelen volt, kérjük próbálja meg újra!<br />";

            toHtml += msg + "<br />";

            toHtml +=
              "Amennyiben többszöri alkalomra sem sikerül a küldés kérem keressen fel minket elérhetőségeink valamelyikén!<br />Köszönettel: G1 csapata";
          } else {
            ...
            toHtml +=
              "Your booking request could not be sent. Please try again!<br />";

            toHtml += msg + "<br />";

            toHtml +=
              "If multiple requests did not succeed, please contact us!<br />Thanks: G1 team";
          }
...
$(document).on("click", ".pack-loader-opener", function () {
  ...
  if (selType !== "" && selPack !== "" && selPack !== "Csomag nélkül") {
    ...
    if (defaultLang) var href = siteUrl + "" + packs[selPack].link;
    else var href = siteUrl + "" + packs[selPack].linken;
